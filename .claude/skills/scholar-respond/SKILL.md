...
The mode is selected from `$ARGUMENTS` (see Dispatch Table). Load ONLY the reference for the
selected mode, substituting the file from the Dispatch Table's **Load** column:

```bash
SKILL_DIR="${SCHOLAR_SKILL_DIR:-.}"
OUTPUT_ROOT="${OUTPUT_ROOT:-output}"
# substitute the selected mode's file:
#   simulate     → references/mode-1-simulate.md
#   respond      → references/mode-2-respond.md
#   revise       → references/mode-3-revise.md
#   resubmit     → references/mode-4-resubmit.md
#   cover-letter → references/mode-5-cover-letter.md
cat "$SKILL_DIR/.claude/skills/scholar-respond/references/mode-1-simulate.md"
```

Do NOT load the other modes' references. Each mode reference loads its own companion references
on demand.
