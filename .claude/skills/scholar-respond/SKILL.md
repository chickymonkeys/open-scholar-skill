...
MODE_REF="$SKILL_DIR/.claude/skills/scholar-respond/references/$REF"
if [ ! -f "$MODE_REF" ]; then
  echo "[scholar-respond] mode reference $REF is missing — cannot proceed without the mode procedure." >&2
  exit 1
fi
cat "$MODE_REF"
```
