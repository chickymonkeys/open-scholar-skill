...
#
# Every direct call to a gate script under ${SCHOLAR_SKILL_DIR:-.}/scripts/gates/
# is probed before execution, so that a missing file is a silent no-op with a
# success exit (the research proceeds on prose alone), while behaviour stays
# byte-identical when the files are present. Three forms are accepted, all
# idioms already in use in this repository:
#
