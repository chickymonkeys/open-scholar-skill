#!/usr/bin/env bash
# Smoke test: the row 3 decomposition keeps scholar-respond inside the documented
# size ceiling, with mode- and dimension-specific material OUT of the always-loaded
# file. The 500-line ceiling is the documented skill-authoring budget (#27 §6); it
# is enforced here because upstream's own gate scripts never check skill size and
# its stubs regrew 14-69% as a result (#27 §0.3).
set -euo pipefail
...
# --- 2. always-loaded share of the component bytes stays far below the 49.4%
#        measured before the decomposition (#26 §0.3, row 3). Component = SKILL.md
#        + every references/*.md; always-loaded = SKILL.md alone. Threshold 25%:
#        generous headroom over the 10.2% measured after the row, far under the
#        measured before, so a stub can regrow without silently re-monolithising.
component=0
