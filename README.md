...
The 36 modular skills provided here are the same building blocks. You are encouraged to build your own workflow by chaining skills in the order that fits your research process. A typical pipeline looks like:
...
| `scholar-respond` | `/scholar-respond` | 5 modes: simulate (3-4 reviewers, plus fifteen routed economics references for economics papers), respond (point-by-point), revise (word-budget), resubmit (rejection retarget), cover-letter |
...
4. Install all 37 skills (36 research skills + the `sync-docs` utility) + 20 agents as **personal skills** in `~/.claude/skills/` and `~/.claude/agents/` — installed per-entry alongside any existing personal skills
